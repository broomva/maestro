...
//    projection OF work with real receipts (state dot, folder, you/loop kind). No session read path is
//    needed: History derives runs from work STATE (project.ts §selectHistory). Empty is honest.
...
//  - Everything else has no user/account read-write path yet, so it is SAMPLE/PREVIEW, labelled: the
//    top-bar chip scopes what is live vs sample; the autonomy-score hero + weekly bars are sample; the
//    Overview Preferences card carries a "preview" chip (its toggles persist nothing); the two inert
//    account-view preference rows (keyboard shortcuts, reduced motion) each carry a "preview" chip so
//    they read distinct from the LIVE Theme control; the identity form says "not saved yet" (not the
//    prototype's false "syncs to your profile"); the score shows receipts (hours / counts / durations),
//    never a fabricated progress percentage (§Work states).
...
//  - Internal navigation is TanStack <Link> (client-side) — never a raw <a href>, which would hard-reload
//    the SPA and tear down the shell's single shared SSE stream (BRO-1824).
//
...
// pure panels are prop-driven + unit-testable. `OverviewPanel` + `AccountPanel` are MODULE-LEVEL
// components (stable identity — never defined in render), so `<OverviewPanel/>` is safe (no remount
// trap), and each half is renderable in isolation so the §Voice guards cover BOTH branches.
...
/** The identity header (avatar + name + role + Edit profile) — module-level, so stable across renders. */
function IdentityHeader({ big, onEdit }: { big: boolean; onEdit: () => void }) {
  return (
...
/** The Overview dashboard — identity + the autonomy-score hero + a two-column body (sessions + prefs). */
export function OverviewPanel({
  ...
  // Local, non-persisted preference toggles — they persist nothing, so the card carries a "preview" chip.
  ...
  return (
    <div className="usr-wrap" data-screen-label="Account · overview">
      <div className="usr-inner">
        <IdentityHeader big onEdit={onEdit} />
        ...
        <div className="usr-cols">
          <div className="usr-card">
            ...
            {sessions.length === 0 ? (
              ...
              sessions.map((s) => {
                ...
                return (
                  <Link className="usr-sess" to="/history" key={s.id}>
                    {v.running ? (
                      <DotComet size={12} />
                    ) : (
                      <span
          ...
          <div className="usr-card">
            <div className="usr-card-head">
              ...
              {/* These toggles persist nothing yet — honest "preview" affordance (the Settings pattern),
                  not a "this device" claim of device-local persistence. */}
            ...
            <div className="usr-prow">
              ...
              <div className="usr-prow-control">
                <SetSwitch label="Digest email" on={digest} onToggle={() => setDigest(!digest)} />
              </div>
            </div>
            <div className="usr-prow">
              <div className="usr-prow-main">
...
/** The editable Account page — identity form, personal preferences (Theme is live), security, devices. */
export function AccountPanel({ onEdit }: { onEdit: () => void }) {
  // Two inert preference toggles — each carries a "preview" chip so they read distinct from live Theme.
  ...
  return (
    <div className="usr-wrap" data-screen-label="Account · account">
      <div className="usr-inner">
        <IdentityHeader big={false} onEdit={onEdit} />
        ...
        <div className="usr-card">
          ...
          <div className="usr-form">
            ...
            <div className="usr-form-field usr-form-field--full">
              ...
              <input id="usr-tz" className="set-input set-input--full" defaultValue={SAMPLE.tz} />
            </div>
          </div>
        </div>

        {/* Personal preferences — Theme is REAL (writes live); the two below persist nothing (preview). */}
        <div className="usr-card">
          <div className="usr-card-head">
          ...
          <div className="usr-prow">
            <div className="usr-prow-main">
              ...
              <span className="usr-prow-desc">Calm monochrome, light or dark. Writes live.</span>
            </div>
            <div className="usr-prow-control">
              <SetSegmented
                label="Theme"
 ...
 * Inspector takes `item` and History takes `sessions`), so it is unit-testable by seeding a fixture. It
 * owns the ephemeral overview/account view toggle; each half is a module-level panel (testable alone).
 */
export function AccountView({ sessions }: { sessions: HistorySession[] }) {
  const [view, setView] = useState<"overview" | "account">("overview");
  const goEdit = () => setView("account");
  ...
  return (
    <div className="usr-page" data-testid="view-account" data-screen-label="Account page">
      ...
      {view === "overview" ? (
        <OverviewPanel sessions={sessions} onEdit={goEdit} />
      ) : (
        <AccountPanel onEdit={goEdit} />
      )}
    </div>
  );
}
