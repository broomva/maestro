...
| `sessionId?` | derived | the node's current-or-most-recent session id — **undefined for ALL never-dispatched states** (`proposed` / `reviewing` / `triggered`) + work canceled before dispatch; present on `running` / `blocked` / `review` / `done`, and on a standing routine once it has fired ≥ once (a session is born only at dispatch, `triggered → running`). The session-timeline join key; **not live-only** |
