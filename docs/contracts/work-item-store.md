...
| `title` | store | = node.title (first heading of `_work.md`). Required though `NodeRow.title` is `string \| null` — the projector (BRO-1775) falls back to the last path segment / `"Untitled"` so the read surface never shows an empty title |
...
1. **`time` (relative string) → an absolute ISO timestamp the client formats (never a stored string).**
   ...
   **Caveat — `updatedAt` is the index-write clock (LWW), not frontmatter `updated`:** it is the epoch
   the index assigned when the `node` row was last written (fs-index, BRO-1754), so a spurious re-index
   must NOT bump it or every card's age resets to "now". Requirement on the indexer (BRO-1796): only
   bump `updatedAt` on an actual content change (a dirty-check on the frontmatter/body), not on every
   scan pass.
...
| the board **cross-group order** | `compareByAttention` + `WK_GROUP_ORDER` already ship in `plain-voice.ts` (state-keyed, no age dimension); this seam adds nothing | — |
| the board **within-group tiebreak** | the key: **recency of attention** (`updatedAt` / a gate's `openedAt`), **NOT `createdAt`** — sorting the attention board by creation time buries freshly-actionable old work | **BRO-1789**, layered on top of the shipped comparator (no fork). Resolves the merged `NodeRow.createdAt` comment, now corrected to stop asserting createdAt is the sort key |
