...
| Decision | What changed | Why | Where |
|---|---|---|---|
| **Widen the event namespaces from five to six — add `agent.*`** | `EVENT_NAMESPACES` gains `agent`; `NamespacedEventType` gains `` `agent.${string}` ``; `EVENT_TYPES` gains `AGENT_SAID` (`agent.said`), plus `RUN_BEAT` (`run.beat`) and `TOOL_RESULT` (`tool.result`) that HARNESS §6 names. DATA-MODEL §A.3 and `packages/protocol/README.md` updated to list six namespaces. | HARNESS §6 maps a coalesced assistant turn to `agent.said`, but the original five namespaces (`run\|tool\|check\|gate\|budget`) had no home for the agent's own utterances — an actual canon discrepancy between HARNESS §6 and DATA-MODEL §A.3. `agent` is already a first-class *actor*, so an `agent.*` type family is the natural fit, and the `events.ts` header already sanctioned "widen the namespace set" as a deliberate resolution the implementing ticket owns. BRO-1756 (the runner-port ticket that first emits these via the SDK→session.jsonl translation) is that ticket. | `packages/protocol/src/events.ts`, `events.test.ts`, `README.md`; `build-docs/DATA-MODEL.md` §A.3 |
